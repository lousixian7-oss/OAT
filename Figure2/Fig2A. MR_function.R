### Compute F and R2 (no EAF version)
get_f_noeaf <- function(dat, F_value = 10) {
  if(is.null(dat$beta.exposure[1]) || is.na(dat$beta.exposure[1])) {
    print("Data does not contain beta, cannot calculate F-statistic")
    return(dat)
  }
  if(is.null(dat$se.exposure[1]) || is.na(dat$se.exposure[1])) {
    print("Data does not contain se, cannot calculate F-statistic")
    return(dat)
  }
  if(is.null(dat$samplesize.exposure[1]) || is.na(dat$samplesize.exposure[1])) {
    print("Data does not contain samplesize, cannot calculate F-statistic")
    return(dat)
  }
...
get_f<-function(dat,F_value=10){
  ...
  if(length(log)==1)
  {if(log==TRUE){
    print("Data does not contain eaf, cannot calculate F-statistic")
    return(dat)}
  }
...
###### Automatically choose IVW random vs fixed effects model #####
